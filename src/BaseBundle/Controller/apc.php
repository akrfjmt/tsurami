...
function sortheader($key,$name,$extra='', $MYREQUEST, $MY_SELF_WO_SORT) {
...
// create menu entry
function menu_entry($ob,$title, $MYREQUEST, $MY_SELF) {
    if ($MYREQUEST['OB']!=$ob) {
...
function put_login_link($MY_SELF,$MYREQUEST,$AUTHENTICATED, $s="Login")
{
...
<div class="head">
    ...
    <div class="login">
        <?php put_login_link($MY_SELF,$MYREQUEST,$AUTHENTICATED); ?>
    </div>
    <hr class="apc">
</div>
...
EOB;
echo
menu_entry(OB_HOST_STATS,'View Host Stats', $MYREQUEST, $MY_SELF),
menu_entry(OB_USER_CACHE,'User Cache Entries', $MYREQUEST, $MY_SELF),
menu_entry(OB_VERSION_CHECK,'Version Check', $MYREQUEST, $MY_SELF);
...
EOB;
        ...
        echo   '<tr class=tr-0><td class=td-0>Uptime</td><td>',duration($cache['start_time'], $time),'</td></tr>';
        echo <<<EOB
		</tbody></table>
		</div>
...
// -----------------------------------------------
    case OB_USER_CACHE:
        if (!$AUTHENTICATED) {
            ...
            put_login_link($MY_SELF,$MYREQUEST,$AUTHENTICATED, "Login now!");
            echo '</div>';
            break;
        }
        $fieldname='info';
...
EOB;
        ...
        '<tr>',
        '<th>',sortheader('S',$fieldheading,  "&OB=".$MYREQUEST['OB'], $MYREQUEST, $MY_SELF_WO_SORT),'</th>',
        '<th>',sortheader('H','Hits',         "&OB=".$MYREQUEST['OB'], $MYREQUEST, $MY_SELF_WO_SORT),'</th>',
        '<th>',sortheader('Z','Size',         "&OB=".$MYREQUEST['OB'], $MYREQUEST, $MY_SELF_WO_SORT),'</th>',
        '<th>',sortheader('A','Last accessed',"&OB=".$MYREQUEST['OB'], $MYREQUEST, $MY_SELF_WO_SORT),'</th>',
        '<th>',sortheader('M','Last modified',"&OB=".$MYREQUEST['OB'], $MYREQUEST, $MY_SELF_WO_SORT),'</th>',
        '<th>',sortheader('C','Created at',   "&OB=".$MYREQUEST['OB'], $MYREQUEST, $MY_SELF_WO_SORT),'</th>';

        if($fieldname=='info') {
            $cols+=2;
            echo '<th>',sortheader('T','Timeout',"&OB=".$MYREQUEST['OB'], $MYREQUEST, $MY_SELF_WO_SORT),'</th>';
        }
        echo '<th>',sortheader('D','Deleted at',"&OB=".$MYREQUEST['OB'], $MYREQUEST, $MY_SELF_WO_SORT),'</th></tr>';
